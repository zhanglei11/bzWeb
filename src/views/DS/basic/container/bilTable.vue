<template>
  <a-card :bordered="false">
    <Query @searchQuery="searchQuery" :type="1" ref="QueryRef" />
  </a-card>
  <a-card :bordered="false">
    <AddBliModal ref="addModalRef" @addSubmit="addSubmit" />
    <VxeTable
      ref="vxeTableRef"
      :tableDate="tableDate"
    ...
    >
      ...
      <template v-slot:tableInfo>
        <vxe-column field="blisterName" :title="$t('basic.容器名称')" sortable></vxe-column>
        <vxe-column field="blisterNo" :title="$t('basic.容器编号')" sortable></vxe-column>
        <vxe-column field="capacity" :title="$t('basic.容量')" sortable> </vxe-column>
        <vxe-column field="unit" :title="$t('basic.容量单位')" sortable>
          <template #default> 个 </template>
        </vxe-column>
...
import AddBliModal from './components/addBliModal.vue'
...
import { blisterPage, blisterDelete } from '@/apis/marketbase'
const formState = ref({})
...
//删除 内容
const onDelete = (row) => {
  blisterDelete({ id: row.id }).then((res) => {
    setMessageTips(res)
    init('initial')
  })
}
